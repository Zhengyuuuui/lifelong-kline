...
var XunhupayService = class {
  ...
  async findReusableInviteDiscountOrder(userId) {
    return withTransaction(async (client) => {
      const result = await client.query(
        `
          SELECT
      ...
      if (existing.payment_status === "pending" && existing.session_fresh && (existing.pay_url || existing.qr_code_url)) {
        return {
      ...
      const cancellationReason = existing.session_fresh ? "payment_session_missing_gateway_url" : "payment_session_expired";
